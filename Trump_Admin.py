...
def update_columns(Trump_Admin, columns):
    for col in columns:
        Trump_Admin[col] = Trump_Admin[col].astype(str)
        Trump_Admin[col] = Trump_Admin[col].str.replace(',', '').str.replace('$', '').astype(float)

update_columns(Trump_Admin, ["Total Dollars", "Total Minority Owned Business Dollars", 
...
chart.set_title("Figure Three: Contracting Percentages to Minority Owned Businesses \n Trump Administration - Executive Departments")
...
chart.set_ylabel("Department Name")
chart.set_title("Figure Four: Annual Contracting Dollars to Minority Owned Businesses - Total Spend \n Trump Administration - Executive Departments")
